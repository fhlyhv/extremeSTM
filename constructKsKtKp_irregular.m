function [Ks,Kt,Kp,Kp0,Ds,Dt,Dp] = constructKsKtKp_irregular(Ltt,Lgt,pt,pp)

% construct structure matrix for the three components of GEV parameters
% Yu Hang, NTU, Jul. 2014
% Ltt - a column vector of latutude
% Lgt - a column vector of logitude
% pt - length of the time series
% pp - period of the time series

% spatial pattern
ps = length(Ltt);
if ps > 1
    Ks = Del_triangle(Ltt,Lgt);
    Ks = Ks*Ks;
    [~,Ds] = eigs(Ks,ps-1);
    Ds = [diag(Ds);0];
else
